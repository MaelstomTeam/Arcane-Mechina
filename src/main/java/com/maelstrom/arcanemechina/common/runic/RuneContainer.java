...
public class RuneContainer
{
	...
	{
		return writeNBT(false);
	}

	public CompoundNBT writeNBT(boolean clear_inventory)
	{
		...
		for (RuneType child : children)
		{
			if (child != null)
			{
				if (clear_inventory && child instanceof RuneType.HoldingRune)
				{
					RuneType.HoldingRune clone = new RuneType.HoldingRune();
					clone.readFromNBT(child.writeToNBT());
					clone.clear();
					list.add(clone.writeToNBT());
				}
				else
